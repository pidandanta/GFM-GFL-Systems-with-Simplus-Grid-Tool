...
for n = 1:NumBr
    % --- 强力注入修复：全网降维为纯代数模型 ---
    R = Rlist(n); X = Xlist(n); B = Blist(n); G = Glist(n);
    
    % 构建静态导纳矩阵 (忽略 sL 和 sC)
    % 对于 AC 系统，静态导纳 Y = G + jB + 1/(R + jX)
    Z_line = R + 1j*X;
    Y_stat = G + 1j*B + 1/Z_line;
    G_eff = real(Y_stat);
    B_eff = imag(Y_stat);
    
    % 强制构建一个无状态变量的静态系统 (D 矩阵承载导纳)
    A_RL = []; B_RL = []; E_RL = [];
    C_RL = []; D_RL = [G_eff, -B_eff; B_eff, G_eff];
    YbrDss{n} = dss(A_RL, B_RL, C_RL, D_RL, E_RL);
    YbrStateStr{n} = {};
    
    continue; % <--- 跳过后面所有的动态计算逻辑 (if/else/dss等)
    ...
    % ============================
        ...
        elseif ( (R==0) && (X==0) ) || isinf(G) || isinf(B)
            ...
            if FB(n)~=TB(n)
                % --- 强制注入修复开始 ---
                if ~( G==0 && B==0 )
                    G = 0; B = 0; % 发现非法的CG，直接在内存里物理清零，不让它报 error
                end
                % error(['Error: Ac mutual branch ' num2str(FB(n)) num2str(TB(n)) ' contains C and/or G']); % 注释掉这一行
                % --- 强制注入修复结束 ---
                
                if X == 0                       % R branch                       % R branch
                    A_RL = []; B_RL = []; E_RL = [];
